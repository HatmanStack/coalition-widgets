/* One fetch and one timer per cadence file, however many widgets are on the page.
 *
...
const POLL = { live: 60_000, weekly: 3_600_000, monthly: 3_600_000, quarterly: 3_600_000, annual: 3_600_000 };

const files = new Map(); // cadence -> { data, error, stale, subscribers, timer }

function state(cadence) {
  if (!files.has(cadence)) {
    files.set(cadence, { data: null, error: null, stale: false, subs: new Set(), timer: null });
  }
